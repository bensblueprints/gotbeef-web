...
export default function CheckoutPage() {
    ...
    try {
      const res = await fetch("/api/checkout/create-intent", {
        ...
        body: JSON.stringify({
          ...
          shipTo: { name, line1, line2: line2 || undefined, city, state, postalCode: postal, country: "US" }
        })
      });
  ...
  // Wait for localStorage to hydrate before deciding if cart is empty.
  // Without this, SSR always renders "empty" and React hydration mismatches.
  if (!cart.hydrated) {
    return <div className="max-w-3xl mx-auto px-6 py-20 text-center text-ink/40">Loading…</div>;
  }
        ...
        <form onSubmit={place} className="space-y-6">
          ...
          <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              ...
              <input required placeholder="Address line 1" className="col-span-2 border border-ink px-4 py-3" value={line1} onChange={e=>setLine1(e.target.value)}/>
              <input placeholder="Apt, suite, unit (optional)" className="col-span-2 border border-ink px-4 py-3 text-ink/70" value={line2} onChange={e=>setLine2(e.target.value)}/>
